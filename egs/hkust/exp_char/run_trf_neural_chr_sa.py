...
from base import *
from trf.isample import trf_sa as trf
...
def get_config(data):
    ...
    config.net_config.update(get_config_cnn(data.get_vocab_size()))
    ...
    config.lr_net = lr.LearningRateTime(1.0, 0.4, tc=1e3)
    ...
    config.load_sampler = os.path.join(wb.mkdir('sampler/lstm_e200_h200x1'), 'model.ckpt')
    config.fix_sampler = True

    config.add_sampler_as_prior = True
...
class OpsForTRF(Operation):
    def __init__(self, m):
        super().__init__(m)
        self.perform_next_epoch = 10.0
        self.perform_per_epoch = 10.0
...
def main():
    ...
    with sv.managed_session(config=session_config) as session:
        with session.as_default():
            ...
            if not wb.exists(config.load_sampler + '.index'):
                print('[TRF] cannot find sampler in {}, pertrain sampling...'.format(config.load_sampler))
                m.sampler.train(data.datas[0], data.datas[1], batch_size=100, lr=config.lr_sampler, max_epoch=10,
                                print_per_epoch=0.1,
                                operation=OpsForSampler(m.sampler, data, logdir),
                                write_model=config.load_sampler)
