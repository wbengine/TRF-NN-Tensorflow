...
def main():
    ...
    if wb.is_window():
        ...
        bindir = '../../tools/srilm'
    fres = wb.FRes('data/models_ppl.txt')  # the result file
    datadir = reader.word_raw_dir()
    nbestdir = reader.word_nbest()
    # print(nbestdir)
    workdir = 'ngramlm/'
    model = ngramlm.Model(bindir, workdir)
    ...
    for order in order_reg:
        ...
        if '-train' in sys.argv or '-all' in sys.argv:
            ...
            model.train(order, write_model, discount='-wbdiscount')
        ...
        if '-rescore' in sys.argv or '-all' in sys.argv:
            model.rescore(write_model, order, nbestdir[0], write_model + '.lmscore')
